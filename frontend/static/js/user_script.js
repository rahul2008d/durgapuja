...
document.addEventListener('DOMContentLoaded', () => {
  ...
  const fixedStartDate = new Date('2024-06-01'); // Example date, replace with your fixed start date

  // Current system date
  const currentDate = new Date();

  // Calculate the difference in days between current date and fixed start date
  const diffInDays = Math.ceil((currentDate - fixedStartDate) / (1000 * 60 * 60 * 24));
  ...
  const currentSubscription = subscriptionDetails.find(subscription => subscription.day === diffInDays);
